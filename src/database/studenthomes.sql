...
--- VIEW FOR MEAL COMBINED WITH USER
CREATE OR REPLACE VIEW `view_meal` AS
SELECT
    `meal`.`StudenthomeID`,
    `meal`.`ID`,
    `meal`.`Name`,
    ...
    `meal`.`OfferedOn`,
    `meal`.`MaxParticipants`,
    `studenthome`.`Name` AS `StudenthomeName`,
    CONCAT(`user`.`First_Name`, ' ', `user`.`Last_Name`) AS `Contact`,
    `user`.`Email`,
    `user`.`Student_Number`
FROM `meal`
LEFT JOIN `user` ON `meal`.`UserID` = `user`.`ID`
LEFT JOIN `studenthome` ON `meal`.`StudenthomeID` = `studenthome`.`ID`;
